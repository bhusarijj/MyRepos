...
export default class EventMonitoring extends LightningElement {
  ...
  handleSubscribe() {
    ...
    if (!this.subChannelMap.get(this.channelName)) {
      //check to see if this is a new channel from the subscribed channel map
      ...
      const messageCallback = (response) => {
        ...
        this.events.push({
          ReplayId: `${sEvent.ReplayId}${Date.now()}`,
          message: sEvent.message,
        }); //push event object
        ...
        if (
          this.isSingleReplayId === true &&
          sEvent.ReplayId === this.replayIdNumber + 1
        ) {
      ...
      subscribe(this.channelName, this.replayIdNumber, messageCallback).then(
        (response) => {
          ...
          if (response) {
            ...
            const evt = new ShowToastEvent({
              message: `Successfully subscribed to : ${JSON.stringify(
                response.channel
              )}`,
              variant: "success",
            });
      ...
      const evt = new ShowToastEvent({
        message: `${JSON.stringify(
          this.channelName
        )} has already been subscribed if you are trying to change Replay Type unsubscribe from channel then resubscribe`,
        variant: "error",
      });
      this.dispatchEvent(evt);
      this.showSpinner = true;
  ...
  handleUnsubscribe() {
    ...
    unsubscribe(this.mapSubscription.get(this.unsubOption), (response) => {
      ...
      const evt = new ShowToastEvent({
        message: `Successfully unsubscribed from : ${JSON.stringify(
